...
interf_df <- bind_rows(lapply(valid_ids, function(i) calculate_diff(all_data, "VR", 0, "time")))
interf_df_ <- bind_rows(lapply(valid_ids, function(i) calculate_diff(all_data, "VR", 1, "time")))
interf_dot_df <- bind_rows(lapply(valid_ids, function(i) calculate_diff(all_data, "desktop", 0, "time")))
interf_s_df <- bind_rows(lapply(valid_ids, function(i) calculate_diff(all_data, "desktop", 1, "time")))
...
combined_diff_t <- rbind(interf_df, interf_df_, interf_dot_df, interf_s_df)


# Gruppieren und Zusammenfassen für combined_diff_info_t
combined_diff_info_t <- combined_diff_t %>%
  group_by(subj, presentation, rel, task) %>%
  summarise(
    diff_irr = mean(diff_irr),
...
combined_diff_t <- combined_diff_t %>%
  group_by(subj) %>%
  summarise(
    diff_irr = mean(diff_irr),
    mean_ratio = mean(mean_ratio),
